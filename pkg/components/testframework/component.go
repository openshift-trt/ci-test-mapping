...
var TestFrameworkComponent = Component{
	Component: &config.Component{
		...
		TestRenames: map[string]string{
			...
			"[Jira:\"Test Framework\"] Prometheus [apigroup:config.openshift.io] when installed on the cluster shouldn't have failing rules evaluation [Suite:openshift/conformance/parallel]":                                                "[sig-instrumentation] Prometheus [apigroup:image.openshift.io] when installed on the cluster shouldn't have failing rules evaluation [Suite:openshift/conformance/parallel]",
			"[Jira:\"Test Framework\"] Prometheus [apigroup:config.openshift.io] when installed on the cluster should have no unexpected alerts in firing state [Early][apigroup:config.openshift.io] [Suite:openshift/conformance/parallel]": "[sig-instrumentation] Prometheus [apigroup:image.openshift.io] when installed on the cluster shouldn't report any alerts in firing state apart from Watchdog and AlertmanagerReceiversNotConfigured [Early][apigroup:config.openshift.io] [Suite:openshift/conformance/parallel]",
		},
	},
}
